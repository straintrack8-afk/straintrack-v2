...
export default function NewDiseaseReportPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log('[NewReport] organization_id being inserted:', userData.organization_id)
            ...
            if (allClinicalSigns.length > 0) {
                await supabase.from('clinical_signs').insert(allClinicalSigns.map(sign => ({ report_id: report.id, sign_name: sign })))
            }
            if (emergencyActions.length > 0) {
                await supabase.from('emergency_actions').insert(emergencyActions.map(action => ({ report_id: report.id, action_name: action })))
            }
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-6 pb-12">
            ...
            <div>
                <h1 className="text-3xl font-bold text-gray-900">New Disease Report</h1>
                <p className="text-gray-600 mt-1">Comprehensive farm-level disease surveillance (22 fields)</p>
            </div>
